...
class ShowVariantXAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (GradleSyncState.getInstance(project).isSyncInProgress) {
            project.notifyVariantX(VariantXBundle.message("notification.syncInProgress"), NotificationType.WARNING)
            return
        }

        // Detect Android app modules
        val appModules = project.service<ModuleDetectionService>().getAppModules()

        if (appModules.isEmpty()) {
            project.notifyVariantX(VariantXBundle.message("notification.noModules"), NotificationType.WARNING)
            return
        }
